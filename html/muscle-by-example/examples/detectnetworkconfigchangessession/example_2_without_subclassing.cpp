...
int main(int argc, char ** argv)
{
   ...
   if (reflectServer.AddNewSession(AbstractReflectSessionRef(&detectSession, false)) != B_NO_ERROR)
   {
      LogTime(MUSCLE_LOG_ERROR, "Couldn't add DetectNetworkConfigChangesSession, aborting!\n");
      return 10;
   }

   MyRandomSession mySession;
   if (reflectServer.AddNewSession(AbstractReflectSessionRef(&mySession, false)) != B_NO_ERROR)
   {
      LogTime(MUSCLE_LOG_ERROR, "Couldn't add MyRandomSession, aborting!\n");
      return 10;
   }
   ...
   else LogTime(MUSCLE_LOG_ERROR, "example_2_without_subclassing is exiting due to an error.\n");
